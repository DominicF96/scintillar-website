...
export default function ResourcesFolderPage() {
  ...
  const breadcrumbs: BreadcrumbItem[] = useMemo(() => {
    ...
    return crumbs;
  }, [pathSegments, isRoot, t.navigation.home]);

  // Get parent folder path
  const parentPath = useMemo(() => {
    if (isRoot) return null;
    if (pathSegments.length === 1) return '/app/resources/home';
    return `/app/resources/${pathSegments.slice(0, -1).join('/')}`;
  }, [pathSegments, isRoot]);

  // Sample data for current folder (in real app, this would be fetched based on currentPath)
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    return filtered;
  }, [sampleData, searchQuery, sortBy, sortOrder]);
  ...
  const handleOpenItem = (item: ResourceItem) => {
    if (item.type === 'folder') {
      const newPath = currentPath ? `${currentPath}/${encodeURIComponent(item.name)}` : encodeURIComponent(item.name);
      router.push(`/app/resources/${newPath}`);
    } else {
