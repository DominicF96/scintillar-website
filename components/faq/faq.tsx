...
import * as i18n from "@/i18n/components/faq.i18n";
import {
  Accordion,
  AccordionItem,
  AccordionTrigger,
...
function FAQ({ locale }: Props) {
  ...
  return (
    <section className="faq-section mx-auto max-w-[1000px] lg:max-w-[1200px] px-8 py-20 pb-32 text-left">
      ...
      <Accordion type="single" collapsible className="mt-8 rounded-md rounded-b-none overflow-hidden">
        {t.faq.items.map((item, index) => (
          <AccordionItem key={index} value={`item-${index}`}>
            <AccordionTrigger>{item.question}</AccordionTrigger>
            <AccordionContent className="font-thin">{item.answer}</AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
    </section>
